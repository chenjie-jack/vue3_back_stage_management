    ...
    <div>
        <el-card class="box-card">
                ...
                <el-table-column label="品牌操作">
                    <template #default="{ row }">
                        <el-button type="primary" size="small" icon="Edit" @click="editTrademark(row)"
                            title="编辑"></el-button>
                        <el-popconfirm :title="`您确定删除已有的${row.tmName}？`" width="250px" icon="Delete" icon-color="red"
            ...
            <el-pagination v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[3, 5, 7, 9]" :small="false"
                :disabled="false" :background="true" layout="prev, pager, next, jumper,->,total, sizes"
                @current-change="getHasTrademark" @size-change="sizeChange" :page-count="9" :total="total" />
        </el-card>
...
const removeTrademark = async (id: number) => {
    ...
    if (result.code == 200) {
        ...
        getHasTrademark(trademarkArray.value.length > 1 ? pageNo.value : pageNo.value - 1)
    } else {
        ElMessage({
            type: 'error',
            message: '删除品牌失败'
