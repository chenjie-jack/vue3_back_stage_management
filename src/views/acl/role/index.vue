...
//接口相关的ts类型
import type { RoleResponseData, Records, RoleData, MenuResponseData, MenuList, PermissionResponseData, RemoveResponseData } from "@/api/acl/role/type"
...
const rules = {
  roleName: [
    { required: true, trigger: 'blur', validator: vaildatorRoleName }
  ]
}
//确定按钮的回调
const save = async () => {
...
const confirm = async () => {
  ...
  //发请求，下发对应的权限
  let result: PermissionResponseData = await reqSetPermission(roleId, permissionId)
  if (result.code == 200) {
    //关闭抽屉
    drawer.value = false
    //提示信息
