  ...
  <el-table :data="permissionArr" style="width: 100%; margin-bottom: 20px" row-key="id" border>
    ...
    <el-table-column label="操作">
      ...
      <template #default="{ row }">
        ...
        <el-popconfirm @confirm="deletePermission(row.id)" :title="`你确定要删除${row.name}?`" width="260px">
          <template #reference>
  ...
  <el-dialog v-model="dialogVisible" :title="menuData.id ? '更新菜单' : '添加菜单'">
    ...
    <el-form>
      <el-form-item label="名称">
        <el-input placeholder="情你输入菜单的名称" v-model="menuData.name"></el-input>
      </el-form-item>
      <el-form-item label="权限值">
        <el-input placeholder="情你输入权限值" v-model="menuData.code"></el-input>
      </el-form-item>
    </el-form>
    <template #footer>
